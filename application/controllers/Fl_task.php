...
class Fl_task extends CI_Controller {
	...
	public function index()
	{
		$query = "	SELECT
					...
					";
		$data['waitpart'] = $this->db->query($query."where part_status = 1 order by created_date")->result();
		$data['cnt_waitpart'] = $this->db->query("select count(*) cnt from (".$query.") db where part_status = 1")->result();
		$data['partpickup'] = $this->db->query($query."where part_status = 2 order by created_date")->result();
		$data['cnt_partpickup'] = $this->db->query("select count(*) cnt from (".$query.") db where part_status = 2")->result();
		$data['escalation'] = $this->db->query($query."where part_status = 3 order by created_date")->result();
		$data['cnt_escalation'] = $this->db->query("select count(*) cnt from (".$query.") db where part_status = 3")->result();
		$data['partreturn'] = $this->db->query($query."where part_status = 5 order by created_date")->result();
		$data['cnt_partreturn'] = $this->db->query("select count(*) cnt from (".$query.") db where part_status = 5")->result();
